...
public class EnemyLevel00Script : MonoBehaviour {
    ...
    public EnemyScript enemyPropeties;

    // Use this for initialization
    void Awake()
    {
        enemyPropeties = GetComponent<EnemyScript>();
        objectLibrary = GameObject.Find("ObjectLibrary");
        mass = 3.2f;
        enginePower = 2f;
        weaponForce = 2;
        waitTime = 1.5f;
        enemyPropeties.shieldPower = 40;
        enemyPropeties.damage = 3;
        currentShieldPower = enemyPropeties.shieldPower;
    ...
    {
        float levelBar = currentShieldPower / enemyPropeties.shieldPower;
    ...
    void OnTriggerEnter2D(Collider2D goCollider)
    {
        ...
        {
            case "PlayerShip":
                goCollider.GetComponent<PlayerShipScript>().currentShieldPower -= enemyPropeties.damage;
                objectLibrary.GetComponent<ObjectLibraryScript>().ReplaceInstance(gameObject);
                break;
            default:
                break;
